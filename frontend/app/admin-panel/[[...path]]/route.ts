...
async function proxy(req: NextRequest, pathSegments: string[] | undefined) {
  ...
  // Always send trailing slash — Django's APPEND_SLASH expects it and
  // the Replit proxy strips incoming trailing slashes before we can inspect them.
  const backendUrl = `${BACKEND}/admin-panel${suffix}/${search}`
  ...
  // Rewrite Location header so redirects stay on the frontend origin
